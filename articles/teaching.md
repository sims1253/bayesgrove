...
This article is for instructors running a Bayesian modeling course on
top of bayesgrove’s decision log. The pedagogical claim is simple:
students learn the workflow loop (Gelman et al. 2020) faster when the
software refuses to let them skip its steps. A fit with divergences
cannot silently feed a model comparison; the student must look at the
evidence and record a judgment, and that judgment is what you grade.
...
summary carries the fingerprint of the configuration that produced it,
the report cannot claim a clean fit the logs do not support. Verifying a
...
rationale — whether “heavier tails to absorb outliers” is justified for
the data at hand — remains the instructor’s job. bayesgrove’s
contribution is that the rationale exists, is attached to the exact
evidence that prompted it, and cannot be fabricated after the fact.
