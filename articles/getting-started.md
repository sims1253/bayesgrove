...
bayesgrove records a Bayesian analysis as a graph of computations plus a
log of decisions. The graph part is familiar from build tools: nodes
hold code, edges hold dependencies, results are cached. The part that
makes it bayesgrove is the review loop on top: fits and checks emit
typed diagnostic *summaries*, summaries raise *obligations*, and a
blocking obligation holds downstream work until you record a *decision*
— a choice plus a rationale — that resolves it. The analysis cannot
quietly run past evidence you never looked at.

This vignette walks the smallest real version of that loop with
cmdstanr: one fit, one posterior-predictive check that fails, the
obligation it raises, the decision that answers it, and the branch that
repairs the model. The chunks use `eval = FALSE` with output pasted from
a real run, so the vignette builds without CmdStan on CI. For the mental
model behind the moving parts, see
[`vignette("concepts")`](https://sims1253.github.io/bayesgrove/articles/concepts.md);
this vignette shows them in motion.
...
The sampler is happy — no divergences, good R-hats.
...
which is exactly why a skewed sample can hide behind a normal model
until you check its extremes. One caveat before moving on:
posterior-predictive p-values are conservative tripwires, not calibrated
tests — an unremarkable p-value is weak evidence of fit. The primary
check is graphical:
...
default pack turns fresh warnings into a blocking obligation — at
project scope it files everything under its baseline review lane,
...
tiny graph, so there is nothing downstream to hold yet. The moment you
add one (a comparison, a report step, a follow-up fit), it lands in
`external_blocked` until the obligation is resolved. The hold mechanism
is how unreviewed evidence stops an analysis from advancing *past* it,
not a punishment for the node that produced it.
...
The obligation is gone — not because the evidence went away, but because
a recorded judgment now covers it. Both stay in the log: a reviewer can
later see exactly which p-value prompted the rejection and what the
stated reasoning was.

## Branch to the repair

The rationale named the repair: model the log scale. A *branch* keeps
the rejected fit and its criticism in the record while the analysis
moves on — provenance is the point, so we never edit the past.
...
Both models remain in the graph — the rejected normal fit with its
failed check and the decision that rejected it, and the lognormal fit
with a clean one:
...
- **Computation review.** When HMC itself misbehaves — divergences, low
  ...
  walks that case, including the branch-scoped review a repair branch
  inherits.
- **Comparison and disposition.** With two or more clean candidate fits,
  the default pack demands an explicit comparison (via LOO) and then an
  ...
  runs that full arc on real data, from a badly misfitting first model
  to an exported report.
- **Reporting.** `bg_export_report(handle, format = "md")` renders the
  whole trail — graph, diagnostics, decisions, branch lineage — into a
  document; a
  [`bg_bundle()`](https://sims1253.github.io/bayesgrove/reference/bg_bundle.md)
  archive backs it with the machine-readable logs. See the
...
below are the pieces in isolation, for when you come back looking for
one of them.

### Gates: pre-planned checkpoints

Obligations arise dynamically from evidence. When you know *in advance*
that a step needs sign-off, put a *gate* on the edge instead. A gate
blocks planning until it is answered, and the answer (with its
rationale) lands in the same decision log:
...
— each active line of analysis answers for its own evidence. Pass
`scope` and `branch_id` to
[`bg_next_actions()`](https://sims1253.github.io/bayesgrove/reference/bg_next_actions.md)
to interrogate a specific branch, as the eight-schools vignette does
after branching.

### Freshness

Summaries are fingerprinted against the exact node configuration that
produced them. Change the params, the data, or the Stan program and the
old summary goes *stale*: it stops feeding obligations until the node is
rerun. This is what makes “the checks passed” meaningful across sessions
and machines.

### Additional workflow packs

[`bg_use_default_workflow()`](https://sims1253.github.io/bayesgrove/reference/bg_use_default_workflow.md)
installs the built-in starter pack used here.
[`bg_use_workflow_packs()`](https://sims1253.github.io/bayesgrove/reference/bg_use_workflow_packs.md)
can instead activate an explicit set, such as the default Bayesian pack
shown below plus the optional prior-workflow pack. These packs add
richer review vocabularies while remaining summary-driven: executors
keep returning plain data, and the packs decide what that evidence
obliges.
...
`bg_repl(handle)` wraps the same loop in an interactive dashboard: it
shows outstanding obligations, previews the suggested actions, and can
execute them. Everything it does goes through the public API you used
above, so a REPL session and a script leave the same audit trail.

### Extending

Custom node kinds, custom executors (including deterministic mock
executors for testing), template registration, and custom summary kinds
are covered in
[`vignette("extensions")`](https://sims1253.github.io/bayesgrove/articles/extensions.md).
The short version: register an executor that returns
`list(result = ..., summaries = ...)` and the review loop picks up your
evidence exactly as it picked up the built-in checks here.
