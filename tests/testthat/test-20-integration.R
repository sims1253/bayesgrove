describe("End-to-End Workflow Integration", {
  it("runs a full workflow with branching, decisions, and cache hits", {
    ...
    bg_register_node_kind(handle, "data_source", executor = function(node, inputs) {
    ...
    mock_cmdstanr <- list(
      backend_compile = function(params) list(compiled = TRUE, id = params$model_id),
      backend_fit = function(compiled, data, params) list(fit = TRUE, chains = params$chains, compiled = compiled),
      backend_source_hash = function(params) params$model_id,
      backend_runtime_signature = function(params) list(fingerprint_fields = list(v = 1))
    )
    ...
    n_prep <- bg_add_node(handle, "transform", label = "Prepped", inputs = n_data)
    
    n_comp_base <- bg_add_node(handle, "compile", params = list(backend = "cmdstanr", model_id = "base"))
    n_fit_base <- bg_add_node(handle, "fit", inputs = c(n_prep, n_comp_base), params = list(backend = "cmdstanr", chains = 4))
    
    # 4. Add a gate
    gate <- bg_add_gate(
      handle, from = n_prep, to = n_fit_base, 
    ...
    bg_update_node(handle, n_fit_branch, params = list(backend = "cmdstanr", chains = 8))
