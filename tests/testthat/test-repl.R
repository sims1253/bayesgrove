...
describe("Interactive REPL", {
  ...
  it("builds node rows with cache and policy-hold details", {
    ...
    bg_run(handle, targets = fixture$fit_id)
  ...
  it("treats guided remediation branches as technical revisions in the demo flow", {
    ...
    run_res <- bg_run(handle, targets = branch_root)
  ...
  it("accepts SBC templates whose source uses a fit node kind", {
    handle <- bg_init(path = withr::local_tempdir())
    ...
    action <- list(
      action_id = "test_sbc_check_fit_kind",
      kind = "create_node_from_template",
      scope = "project",
      title = "Create SBC check",
      basis = list(node_ids = fit_id),
    ...
    result <- bg_apply_template_action(
      handle,
      action,
      scope = "project",
      interactive = FALSE
  ...
  it("supports the scripted demo flow through comparison and branch disposition", {
    ...
    run_res <- bg_run(handle, targets = revised_root)
    expect_equal(run_res$status, "succeeded")
    ...
    compare_run <- bg_run(handle, targets = comparison$node_id)
