...
Bayesian analysis is iterative by design. You fit a model, inspect
diagnostics, revise your assumptions, branch into alternatives, compare
candidates, and eventually commit to one path. The problem is that this
process quietly disappears into unstructured scripts. Three months
later—or when a collaborator picks up the project—the *why* behind every
modeling decision is gone.

**bayesgrove** is a graph-based workflow orchestrator for iterative
Bayesian analysis in R. While general-purpose pipelines like `targets`
are built to skip up-to-date computations, bayesgrove is built to
enforce statistical workflow. It separates the *execution graph* (the
DAG of computations that run) from the *decision log* (why you ran
them), and enforces a structured review loop: diagnostic warnings block
downstream work, model criticism is recorded before repair, clean
candidates are formally compared before one is accepted. `targets`
doesn’t know what a divergent transition is; bayesgrove does, and it
will block your posterior predictive checks until you formally record
...
- **Content-aware caching.** Each node carries a SHA-256 fingerprint of
  its inputs and parameters. Expensive MCMC fits are skipped when
  nothing upstream changed semantically. Change a prior and only the
  affected subtree reruns.

- **Enforced review.** The built-in workflow protocol derives
  obligations from the summaries your executors return. Warning
  diagnostics create a `review_computation_validity` obligation.
  Branch-scoped fit warnings also trigger `review_fit_criticism`. Once
  two candidates are clean, the protocol requires an explicit
  `compare_candidate_branches` step. Each branch then needs an
  `accept_or_reject_branch` disposition before the loop closes.

- **Branching with full lineage.** Fork from any node without losing the
  baseline. Branch goals, decisions, and diagnostic history travel with
  the branch. The lineage of any branch is always recoverable.

- **Reproducible handoffs.** Export the full decision tree and graph
  topology into a portable bundle or a structured markdown report with a
...
and internal-but-exported functions. Use `bg_api_boundary()` to inspect
that classification directly from R, including which functions are part
of the smaller remote IPC contract exposed through the experimental
`bg_serve()` server.
...
you always would, and return a `summaries` list alongside the fit.
*(Note: Future versions of bayesgrove will include built-in `cmdstanr`
and `brms` executors to reduce boilerplate. This example builds one from
scratch to show the mechanics).* bayesgrove reads those summaries to
derive workflow obligations, cache validity, and holds. The rest of the
machinery (DAG, fingerprinting, branching) is invisible until you need
it.
...
The bayesgrove workflow is shaped by the iterative model-building
perspective described in Gelman et al. (2020, *Bayesian Workflow*,
[arXiv:2011.01808](https://arxiv.org/abs/2011.01808)) and the
simulation-based calibration approach of Talts et al. (2018,
[arXiv:1804.06788](https://arxiv.org/abs/1804.06788)). The obligation
and decision layers take direct inspiration from the practical need to
record the *why* behind each modeling step, not just the *what*—so that
analysis lineage is an artifact of the workflow, not an afterthought.
