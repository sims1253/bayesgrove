...
`bg_next_actions()` and `bg_plan(..., external_holds = ...)`, but that layer is
kept out of this vignette so the simulation example stays focused.
...
bg_answer_gate(
  ...
  randomized$gate$id,
  choice = "no",
  rationale = "The standardized mean difference for z is small, so the adjustment branch is not required to justify interpretation here.",
...
bg_answer_gate(
  ...
  confounded$gate$id,
  choice = "yes",
  rationale = "Treatment assignment is visibly associated with z, so the adjusted branch is the only trustworthy explanatory candidate.",
...
```{r report}
report_path <- bg_export_report(handle, out_file = "simulation-study-report.md")
report_path
```
...
This vignette demonstrates the shape of a guided workflow, but not yet the full
Bayesian system you ultimately want. In particular, the current package still
lacks first-class support for:
...
Those pieces need a richer domain layer on top of the current execution and
