...
<!-- The pasted output blocks below were captured from a real run via
     tools/precompute-case-study-roaches.R (sampler progress lines and
     repeated warnings trimmed). Regenerate and refresh the blocks when
     bayesgrove or CmdStan changes; outputs are version-dependent.
     Last refreshed against bayesgrove 0.7.0 / CmdStan 2.39.0. -->

This is the package's flagship case study: a real dataset, a real inferential
question, and the full review loop from a badly misfitting first model to an
exported report. Unlike a reference walkthrough, it shows the loop doing its
actual job — the enforced reviews *change the analysis*, and the final answer
differs substantively from what the first model would have reported.
...
vignette builds without CmdStan on CI. Everything here is reproducible: the
dataset ships with the package, as do the three Stan programs.
...
Here is the payoff. The treatment effect, as a multiplicative factor on the
...
A naive analysis that stopped at the clean-sampling Poisson fit would have
reported a 40 percent roach reduction with a strikingly precise interval —
plus or minus a couple of points. The accepted model reports a *larger*
estimated reduction with an honest interval roughly five times wider. The
Poisson's precision was an artifact of an equidispersion assumption the
data falsify at p-value zero. Nothing about this repair required the
workflow tooling — but the tooling made it *non-optional*: the failed check
raised a blocking obligation, the obligation demanded a recorded judgment,
and the judgment is now part of the project's permanent record rather than
a private glance at a plot.
...
```{r report}
bg_export_report(handle, format = "md", path = "roaches-report.md")
```
