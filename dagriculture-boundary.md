...
`docs/architecture/dagriculture-boundary.md` so pkgdown can expose it as a
...
| Public graph editing verbs such as `bg_add_node()` and `bg_connect()` | BayesGrove | User-facing API shape, ID generation, persistence, and project commit semantics belong to this package even when they delegate structural mutations. |
