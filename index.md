...
- **Optional Bayesian semantics packs.** Start with a small default
  review loop, then add prior rationale, prior and posterior predictive
  checks, SBC review, stacking-aware model selection, minimal causal
  framing, and PAD annotations when you need them.
...
of internal-but-exported helpers. Use
[`bg_api_boundary()`](https://sims1253.github.io/bayesgrove/reference/bg_api_boundary.md)
to inspect that boundary from R, including the smaller remote IPC
contract exposed through the experimental
...
- `bayesgrove.model_checks` adds posterior predictive review, and on
  branch-scoped `latent_inference` goals it also asks for SBC.
...
- `bayesgrove.causal_minimal` adds an advisory branch-scoped
  causal-framing prompt for `latent_inference` branches.
- `bayesgrove.pad_scaffold` adds an advisory PAD annotation prompt once
  a branch has an inferential goal.
...
for a guide to writing node sets, domain modules, and backend plugins
